...
> 依据文件：`SKILL.md`、`references/engine/*.md`、`domains/unity-csharp/*`、`docs/DESIGN-AUDIT.md`、**真实在学状态 `.coach/state.json`（10 条证据）与 `.coach/scaffold/`**。
> 已实跑：`coach-validate.mjs`（真实状态与 demo 夹具均 `ok:true`；真实状态 `schemaVersion=1.0`）。
...
| F3 | 需修改 | 必要性成立：真实状态 E7/E8/E9/E10 的 artifact 本就是「用户消息中的作答原文」，`example.md:111` 亦然——**现行口径与实际做法不符**；但阈值与 artifact 规则须重写（阻塞①、重要③） |
...
- **真实对照（关键证据）**：真实 `.coach/state.json` 的 E9 note 写着「尚未附 Unity 实测截图，实测后升级为已验证」；`.coach/scaffold/README.md:42-52` 同样如此。
...
**④ F8 会让在学状态直接失效**
- 真实 `.coach/state.json` 为 `schemaVersion=1.0`、10 条 evidence 无 `kind`，现跑 `ok:true`；`coach-validate.mjs:196-198` 是 `!==` 精确比较，`:302` 的 `requireKeys` 会逐条报 ST-MISSING；而 `SKILL.md:45` 要求校验失败即停下修状态。
