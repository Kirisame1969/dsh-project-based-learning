...
> **本维度共 4 道题**：Q1-2、**Q1-3 与 Q1-4（均首触可用，前置为空）**（均在下方）与 **Q1-1（已移入《讲授后确认题》）**。Q1-1 属**事实性**题，零基础学员先按 R9 讲授再用它确认；自述熟悉生命周期的学员可直接用它诊断——是否可用由 `前置知识` 与学员记录决定，见《取题前置检查》。
...
**题目**
写一个最小的组件：挂在任意对象上，**每隔 N 秒**在 Console 打印一次当前累计秒数，`N` 可以在 Inspector 里调整（默认 1 秒）。

要求：只用你能说清的部分；能写出伪代码或分步思路也算部分完成——但要说明**时间从哪里来**、**在哪里累加**、**怎么判断"到点了"**。

**考察维度**：基础知识
**最小诊断类别**：小型实现
**类型**：综合
**前置知识**：无（可首触：只需能写字段与方法体，不要求预先了解任何引擎 API）

**合格回答要点**
- 至少给出"一个随时间累加的变量 + 一个阈值比较 + 到点后重置"的结构，并说明累加依赖**每帧/每固定步长的时间增量**（而不是自己数帧数）。
- 字段要能在 Inspector 调整：`public` 字段或 `[SerializeField] private`（与 Q1-3 同源）。
- 能说清"到点后把累计值**减去**阈值还是**清零**"的区别（前者不丢余量，后者会漂移）。
- 知道应把"读输入/渲染相关"与"计时"放在合适的更新时机里；至少能说明为什么不能用帧数当时间。

**典型错误回答**
- 用帧数当时间（"每 60 帧算 1 秒"），并认为这与帧率无关。
- 累加后不清零也不减阈值，导致每秒打印次数越来越多。
- 说不清时间从哪来，只会照抄一个 `Update` 里加常量的写法。

**加难追问**
如果游戏暂停（时间缩放设为 0），你的计时器应该继续走还是停？两种选择各要改哪里？

**降难追问**
只回答一个问题：要让"每隔 N 秒"成立，你需要在每一帧做哪两件事？

**1–5 等级锚点**
- 1：写不出结构，或时间来源完全说不清。
- 2：能写出累加与比较，但时间来源错误（数帧）。
- 3：结构正确、时间来源正确（用时间增量），但没考虑余量或暂停。
- 4：结构、时间来源、余量处理都正确，字段可在 Inspector 调整。
- 5：能主动讨论暂停语义与更新时机的取舍，并给出验证方式（例如打印间隔实测）。
...
>
> **首触可用题**（`前置知识` 为"无（可首触）"，可直接用于首次诊断）：**Q1-3（问题定位）与 Q1-4（小型实现）**——两者可直接凑齐"问题定位 + 小型实现"两类；"理解预测"一类由索引中的 Q2-1／Q3-2／Q7-1 承担（各有前置，需先核对）。零基础学员仍按上一条走 R9 路径。
